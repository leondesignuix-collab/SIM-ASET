...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row antialiased font-sans">
      ...
      <aside className={`w-full ${isSidebarCollapsed ? 'md:w-20' : 'md:w-64'} bg-slate-900 flex flex-col border-r border-slate-800 text-slate-200 shrink-0 select-none shadow-xl md:sticky md:top-0 md:h-screen transition-all duration-300 ease-in-out`}>
        
        {/* Sidebar Header Brand segment */}
        <div className={`p-4 border-b border-slate-800 flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between'} transition-all`}>
          {!isSidebarCollapsed && (
            <div className="flex items-center gap-2.5 animate-fade-in shrink-0">
              <div className="w-8.5 h-8.5 bg-emerald-500 rounded-lg flex items-center justify-center text-white shadow-lg shadow-emerald-500/20 shrink-0">
          ...
          {isSidebarCollapsed && (
            <div className="w-9 h-9 bg-emerald-500 rounded-lg flex items-center justify-center text-white shadow-lg shadow-emerald-500/20 shrink-0 cursor-pointer" onClick={toggleSidebar} title="SIMAS Gereja (Perbesar)">
              <Church className="w-5 h-5" />
            </div>
          )}
          ...
          <button 
            ...
            className={`hidden md:flex items-center justify-center w-6 h-6 rounded-md bg-slate-850 hover:bg-slate-800 text-slate-400 hover:text-slate-200 transition-colors ${isSidebarCollapsed ? 'ml-0' : ''}`}
        ...
        <nav className="flex-1 p-3 space-y-1.5 overflow-y-auto">
          ...
          <button
            ...
            className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center py-3' : 'gap-3 py-2.5 px-3'} rounded-lg text-[11px] font-bold uppercase tracking-wider transition ${
              activeTab === 'dashboard'
                ...
                : 'text-slate-400 hover:bg-slate-800/80 dark:hover:bg-slate-800/80 hover:text-slate-200'
            }`}
          >
          ...
          >
            ...
            {!isSidebarCollapsed ? (
              ...
              <span className="absolute top-1 right-2 bg-emerald-600 text-white font-mono text-[8px] px-1 rounded-full">{assets.length}</span>
            )}
          </button>
          ...
          <button
            ...
            className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center py-3 text-emerald-500 hover:bg-slate-800' : 'gap-3 py-2.5 px-3'} rounded-lg text-[11px] font-bold uppercase tracking-wider transition ${
              activeTab === 'qr'
                ? 'bg-emerald-500/10 text-emerald-400 border-l-4 border-emerald-500 font-bold'
                : 'text-slate-400 hover:bg-slate-800/80 dark:hover:bg-slate-800/80 hover:text-slate-200'
            }`}
          ...
          {!isSidebarCollapsed ? (
            <div className="border-t border-slate-800/60 my-4 pt-3">
              ...
              <div className="px-2 py-2 bg-slate-950/60 rounded-lg border border-slate-850 space-y-1.5 text-[10px]/snug text-slate-400 font-mono">
                <div className="text-slate-500">EMAIL:</div>
                <div className="text-emerald-450 truncate font-semibold">{currentUser.email}</div>
            ...
            <div className="border-t border-slate-800/60 my-4 pt-3 flex flex-col items-center gap-2">
              <div className="w-7 h-7 bg-slate-950 text-emerald-400 border border-slate-800 font-bold font-mono rounded-full flex items-center justify-center text-[10px] transition-transform hover:scale-105 cursor-pointer" title={`Operator: ${currentUser.name}`}>
          ...
          <div className="p-4 border-t border-slate-800 bg-slate-950/40">
            <div className="flex flex-col gap-2">
              ...
              <select
                ...
                className="w-full text-xs font-semibold text-slate-300 bg-slate-800 border border-slate-700 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-emerald-500 cursor-pointer"
              >
                {users.map(user => (
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex flex-col md:flex-row items-center justify-between px-6 gap-3 shrink-0 py-3 md:py-0 sticky top-0 z-10 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            {currentUser.role === 'SUPER_ADMIN' ? (
              <span className="bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded text-[10px] uppercase font-bold border border-emerald-100 flex items-center gap-1 shadow-sm font-mono">
